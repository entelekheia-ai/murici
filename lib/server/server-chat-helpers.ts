...
//
// Uses "||", not "??", for every string field below: a saved profile can
// carry "" instead of null for a key the user never typed into (see
// profile-settings.tsx, whose local state defaults to
// useState(profile?.x_api_key || "") and saves that straight back) — "??"
// would treat that "" as "a key was provided" and never fall back to the
// env var. use_azure_openai stays "??" since false is a meaningful value
// there, not an unset placeholder.
